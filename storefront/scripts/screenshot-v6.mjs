...
// 1. Header retail completo (las 3 capas)
const t1 = await browser.newPage()
await t1.goto(`${BASE}/`, { waitUntil: "networkidle0" })
await new Promise((r) => setTimeout(r, 1500))
await t1.setViewport({ width: 1440, height: 700 })
await t1.screenshot({ type: "png" }).then((b) => writeFile(`${OUT}/v6-01-header-retail.png`, b))
console.log("✓ header retail 3 capas")
await t1.close()
...
await new Promise((r) => setTimeout(r, 1500))
await t2.setViewport({ width: 1440, height: 800 })
const btn = await t2.waitForSelector('button[aria-haspopup="true"]', { timeout: 5000 })
await btn.hover()
await new Promise((r) => setTimeout(r, 800))
await t2.screenshot({ type: "png" }).then((b) => writeFile(`${OUT}/v6-02-header-megamenu.png`, b))
...
await new Promise((r) => setTimeout(r, 1500))
await t3.screenshot({ fullPage: true, type: "png" }).then((b) => writeFile(`${OUT}/v6-03-home.png`, b))
...
await t4.screenshot({ fullPage: true, type: "png" }).then((b) => writeFile(`${OUT}/v6-04-tienda.png`, b))
console.log("✓ tienda")
await t4.close()
